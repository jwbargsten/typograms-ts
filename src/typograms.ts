...
function grid(width, height) {
  ...
  const horizontal = createTag("line", {
...
":" : ([top, right, bottom, left, topRight, bottomRight, bottomLeft, topLeft]) => {
  ...
  line "y2":  "60",
  line "class":  "part",
  line "style":  "stroke-dasharray: 15; stroke-dashoffset: 0;",
  result.appendChild(line);
  if (top == "+") {
    const line = document.createElementNS(
      "http://www.w3.org/2000/svg", "line");
...
"o": ([top, right, bottom, left, topRight, bottomRight, bottomLeft, topLeft]) => {
  const result = document.createElementNS(
      "http://www.w3.org/2000/svg", "g");
  const circle = document.createElementNS(
      "http://www.w3.org/2000/svg", "circle");
...
function create(source, zoom, debug) {
  ...
  svg "width":  width * 30 * zoom,
  svg "height":  height * 54 * zoom,
  svg "debug":  debug,
  const padding = 0;

  svg "viewBox":  `${-padding} ${-padding} ${width * 30 + 2 * padding} ${height * 54 + 2 * padding}`,
  svg "class":  "debug",
